...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					...
					'<p><strong>' . __( 'Note: Since this tool only gathers data from WLP and participating plugins, you may need to do more to comply with export requests. For example, you should also send the requester some of the data collected from or stored with the 3rd party services your organization uses.' ) . '</strong></p>',
	)
);
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'WLP collects (but <em>never</em> publishes) a limited amount of data from registered users who have logged in to the site. Generally, these users are people who contribute to the site in some way -- content, store management, and so on. With rare exceptions, these users do not include occasional visitors who might have registered to comment on articles or buy products. The data WLP retains can include:' ) . '</p>' .
...
get_current_screen()->add_help_tab(
	array(
		...
		'content' =>
					'<p>' . __( 'Many plugins may collect or store personal data either in the WLP database or remotely. Any Export Personal Data request should include data from plugins as well.' ) . '</p>' .
