...
class WP_Automatic_Updater {
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			case 'core':
				...
				$skin->feedback( __( 'Updating to WLP %s' ), $item->version );
				/* translators: %s: WordPress version. */
				$item_name = sprintf( __( 'WLP %s' ), $item->version );
				break;
			case 'theme':
			...
			case 'translation':
				...
				/* translators: 1: Project name (plugin, theme, or WLP), 2: Language. */
		...
		if ( 'core' === $type ) {
			...
			if ( is_wp_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'WLP updated successfully.' ) );
			}
		}
	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success': // We updated.
				/* translators: Site updated notification email subject. 1: Site title, 2: WLP version. */
				$subject = __( '[%1$s] Your site has updated to WLP %2$s' );
				break;

			case 'fail':   // We tried to update but couldn't.
			case 'manual': // We can't update (and made no attempt).
				/* translators: Update available notification email subject. 1: Site title, 2: WLP version. */
				$subject = __( '[%1$s] WLP %2$s is available. Please update!' );
				break;
		...
		switch ( $type ) {
			case 'success':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: WLP version. */
					__( 'Howdy! Your site at %1$s has been updated automatically to WLP %2$s.' ),
					home_url(),
					$core_update->current
				);
				$body .= "\n\n";
				...
				/* translators: %s: WLP version. */
				$body .= sprintf( __( 'For more on version %s, see the About WLP screen:' ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					/* translators: %s: WLP latest version. */
					$body .= "\n\n" . sprintf( __( 'WLP %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'manual':
				$body .= sprintf(
					/* translators: 1: Home URL, 2: WLP version. */
					__( 'Please update your site at %1$s to WLP %2$s.' ),
					home_url(),
			...
			case 'critical':
				if ( $newer_version_available ) {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: WLP version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update WLP to version %2$s.' ),
						home_url(),
						$core_update->current
					);
				} else {
					$body .= sprintf(
						/* translators: 1: Home URL, 2: WLP latest version. */
						__( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of WLP, %2$s.' ),
						home_url(),
						$core_update->current
					);
				}
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf(
				...
				__( 'The WLP team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working.' ),
				$core_update->support_email
			);
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the WLP.net support forums may be able to help.' );
		...
		if ( $critical_support ) {
			$body .= ' ' . __( "Reach out to WLP Core developers to ensure you'll never have this problem again." );
		}
	...
	protected function send_plugin_theme_email( $type, $successful_updates, $failed_updates ) {
		...
		// Add a note about the support forums.
		$body[] = __( 'If you experience any issues or need support, the volunteers in the WLP.net support forums may be able to help.' );
		$body[] = __( 'https://whitelabelpress.org/' );
		$body[] = "\n" . __( 'The WLP Team' );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'WLP site: %s' ), network_home_url( '/' ) );
		...
		if ( isset( $this->update_results['core'] ) ) {
			...
			if ( $result->result && ! is_wp_error( $result->result ) ) {
				/* translators: %s: WLP version. */
				$body[] = sprintf( __( 'SUCCESS: WLP was successfully updated to %s' ), $result->name );
			} else {
				/* translators: %s: WLP version. */
				$body[] = sprintf( __( 'FAILED: WLP failed to update to %s' ), $result->name );
				$failures++;
			}
...
This debugging email is sent when you are using a development version of WLP.

If you think these failures might be due to a bug in WLP, could you report it?
