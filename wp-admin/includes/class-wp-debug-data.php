<?php
/**
 * Class for providing debug data based on a users WLP environment.
 *
 * @package WLP
...
class WP_Debug_Data {
	...
	public static function debug_data() {
		...
		if ( is_array( $core_updates ) ) {
			foreach ( $core_updates as $core => $update ) {
				if ( 'upgrade' === $update->response ) {
					/* translators: %s: Latest WLP version number. */
		...
		$info['wp-dropins'] = array(
			...
			'description' => sprintf(
				...
				__( 'Drop-ins are single files, found in the %s directory, that replace or enhance WLP features in ways that are not possible for traditional plugins.' ),
		...
		$info['wp-constants'] = array(
			'label'       => __( 'WLP Constants' ),
			'description' => __( 'These settings alter where and how parts of WLP are loaded.' ),
			'fields'      => array(
				'ABSPATH'             => array(
					'label'   => 'ABSPATH',
					'value'   => ABSPATH,
		...
		$info['wp-filesystem'] = array(
			...
			'description' => __( 'Shows whether WLP is able to write to the directories it needs access to.' ),
			'fields'      => array(
				'root'  => array(
					'label' => __( 'The main WLP directory' ),
		...
		if ( ! $is_multisite ) {
			...
			$info['wp-paths-sizes']['fields'] = array(
				'classicpress_path' => array(
					'label' => __( 'WLP directory location' ),
					'value' => untrailingslashit( ABSPATH ),
				),
				'classicpress_size' => array(
					'label' => __( 'WLP directory size' ),
					'value' => $loading,
					'debug' => 'loading...',
				),
				'uploads_path'   => array(
		...
		if ( is_file( ABSPATH . '.htaccess' ) ) {
			...
			// Filter away the core WLP rules.
			$filtered_htaccess_content = trim( preg_replace( '/\# BEGIN WLP[\s\S]+?# END WLP/si', '', $htaccess_content ) );
			...
			if ( $filtered_htaccess_content ) {
				...
				/* translators: %s: .htaccess */
				$htaccess_rules_string = sprintf( __( 'Your %s file contains only core WLP features.' ), '.htaccess' );
			}
	...
	/**
	 * Fetches the sizes of the WLP directories: `wordpress` (ABSPATH), `plugins`, `themes`, and `uploads`.
