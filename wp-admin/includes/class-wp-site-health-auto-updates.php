<?php
/**
 * Class for testing automatic updates in the WLP code.
 *
 * @package WLP
...
class WP_Site_Health_Auto_Updates {
	...
	public function test_check_wp_filesystem_method() {
		...
		if ( ! $success ) {
			$description  = __( 'Your installation of WLP prompts for FTP credentials to perform updates.' );
		...
		return array(
			'description' => __( 'Your installation of WLP does not require FTP credentials to perform updates.' ),
			'severity'    => 'pass',
		);
	}
	...
	public function test_all_files_writable() {
		...
		if ( ! $checksums ) {
			$description = sprintf(
				...
				__( "Couldn't retrieve a list of the checksums for WLP %s." ),
				$cp_version
			);
			$description .= ' ' . __( 'This could mean that connections are failing to WordPress.org or WLP.net.' );
			return array(
				'description' => $description,
				'severity'    => 'warning',
			);
		...
		if ( $unwritable_files ) {
			...
			return array(
				'description' => __( 'Some files are not writable by WLP:' ) . ' <ul><li>' . implode( '</li><li>', $unwritable_files ) . '</li></ul>',
				'severity'    => 'fail',
			);
		} else {
			return array(
				'description' => __( 'All of your WLP files are writable.' ),
				'severity'    => 'pass',
			);
		}
	}
	...
	public function test_accepts_dev_updates() {
		...
		if ( defined( 'WP_AUTO_UPDATE_CORE' ) && ( 'minor' === WP_AUTO_UPDATE_CORE || false === WP_AUTO_UPDATE_CORE ) ) {
			return array(
				'description' => sprintf(
					...
					__( 'WLP development updates are blocked by the %s constant.' ),
		...
		if ( ! apply_filters( 'allow_dev_auto_core_updates', $wp_version ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'WLP development updates are blocked by the %s filter.' ),
	...
	public function test_accepts_minor_updates() {
		if ( defined( 'WP_AUTO_UPDATE_CORE' ) && false === WP_AUTO_UPDATE_CORE ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the constant used. */
					__( 'WLP security and maintenance releases are blocked by %s.' ),
		...
		if ( ! apply_filters( 'allow_minor_auto_core_updates', true ) ) {
			return array(
				'description' => sprintf(
					/* translators: %s: Name of the filter used. */
					__( 'WLP security and maintenance releases are blocked by the %s filter.' ),
