<?php
/**
 * WLP Dashboard Widget Administration Screen API
 *
 * @package WLP
...
function wp_dashboard_setup() {
    ...
    // NEW WLP Dashboard Widget
    if ( is_blog_admin() && current_user_can( 'manage_options' ) ) {
        wp_add_dashboard_widget(
            'plugins_themes_overview_widget', // Widget slug.
            'Plugins and Themes Overview', // Title of the widget.
            'display_plugins_themes_overview' // Callback function.
        );
    }
...
function wp_dashboard_primary() {
	$feeds = array(
		'news'   => array(
			...
			/**
			 * Filters the primary link title for the 'WLP News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's primary link.
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'WLP Blog' ) ),
			'items'        => 5,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $is_IE ) {
			$msg = __( 'Internet Explorer does not give you the best WLP experience. Switch to Microsoft Edge, or another more modern browser to get the most from your site.' );
		} elseif ( $response['insecure'] ) {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best WLP experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			$msg = sprintf(
				/* translators: %s: Browser name and link. */
				__( "It looks like you're using an old version of %s. For the best WLP experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
...
function wp_dashboard_php_nag() {
	...
	<p>
		<?php _e( 'PHP is one of the programming languages used to build WLP. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?>
		<?php
...
function display_plugins_themes_overview() {
    // Count installed plugins
    $all_plugins = get_plugins();
    $total_plugins = count($all_plugins);
    $active_plugins = count(array_filter($all_plugins, 'is_plugin_active'));
    $outdated_plugins = 0;
    ...
    // Display the counts
    echo "<h2><img src=\"./images/wlp-32.png\" style=\" margin: -7px -3px; \"> Plugins ({$total_plugins})</h2>";
    echo "<ul>
            <li>Updates: <b>{$outdated_plugins}</b></li>
          </ul>";
    echo "<h2><img src=\"./images/wlp-32.png\" style=\" margin: -7px -3px; \"> Themes ({$total_themes})</h2>";
    echo "<ul>
            <li>Updates: <b>{$outdated_themes}</b></li>
          </ul>";
}
