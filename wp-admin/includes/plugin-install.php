<?php
/**
 * WLP Plugin Install Administration API
 *
 * @package WLP
...
/**
 * Retrieves plugin installer pages from the WLP.net Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the WLP.net
 ...
 *         @type bool $requires          Whether to return the required WLP version. Default true.
 ...
 *         @type bool $compatibility     Whether to return the WLP compatibility list. Default true.
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the WLP.net Plugin Installation API arguments.
	 *
	...
	/**
	 * Filters the response for the current WLP.net Plugin Installation API request.
	 *
			...
			if ( ! wp_is_json_request() ) {
				trigger_error(
					sprintf(
						...
						__( 'An unexpected error occurred. Something may be wrong with WordPress.org, WLP.net or this server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.' ),
						__( 'https://forums.classicpress.net/c/support' )
					) . ' ' . __( '(WLP could not establish a secure connection to WordPress.org. Please contact your server administrator.)' ),
...
/**
 * Retrieves popular WLP plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_plugins_favorites_form() {
	...
	?>
	<p><?php _e( 'If you have marked plugins as favorites on WLP.net, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites">
		<p>
			<label for="user"><?php _e( 'Your WLP.net username:' ); ?></label>
	...
	switch ( current_filter() ) {
		case 'install_plugins_beta':
			printf(
				...
				'<p>' . __( 'You are using a development version of WLP. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	...
	if ( ! $tested_wp ) {
		...
		_e( '<strong>Warning:</strong> This plugin <strong>has not been tested</strong> with your current version of WLP.' );
		echo '</p></div>';
	} elseif ( ! $compatible_wp ) {
		echo '<div class="notice notice-error notice-alt"><p>';
		_e( '<strong>Error:</strong> This plugin <strong>requires a newer version of WLP</strong>.' );
		if ( current_user_can( 'update_core' ) ) {
			printf(
				/* translators: %s: URL to WordPress Updates screen. */
				' ' . __( '<a href="%s" target="_parent">Click here to update WLP</a>.' ),
