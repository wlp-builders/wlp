 ...
 *     Requires at least: Optional. Specify the minimum required WLP version.
 ...
 *     @type string $RequiresWP  Minimum required version of WLP.
 *     @type string $RequiresCP  Minimum required version of WLP.
 ...
 *
 * WLP only supports plugin files in the base plugins directory
 ...
 *
 * WLP only includes mu-plugin files in the base mu-plugins directory (wp-content/mu-plugins).
 *
 * @since 3.0.0
...
/**
 * Returns drop-ins that WLP uses.
 *
...
function delete_plugins( $plugins, $deprecated = '' ) {
	...
	if ( empty( $plugins_dir ) ) {
		return new WP_Error( 'fs_no_plugins_dir', __( 'Unable to locate WLP plugin directory.' ) );
	}
...
/**
 * Validates the plugin requirements for WLP version and PHP version.
 *
	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			'<p>' . sprintf(
				...
				_x( '<strong>Error:</strong> Current versions of WLP (%1$s) and PHP (%2$s) do not meet minimum requirements for %3$s. The plugin requires WordPress %4$s and PHP %5$s.', 'plugin' ),
		...
		return new WP_Error(
			...
			'<p>' . sprintf(
				...
				_x( '<strong>Error:</strong> Current WLP version (%1$s) does not meet minimum requirements for %2$s. The plugin requires WordPress %3$s.', 'plugin' ),
				get_bloginfo( 'version' ),
				$plugin_headers['Name'],
				$requirements['requires']
			) . '</p>'
 ...
 * upgrade due to incompatibility with the current version of WLP.
 *
 * @since 5.8.0
 * @access private
 *
 * @global string $pagenow    The filename of the current screen.
 * @global string $wp_version The WLP version string.
 */
	...
	foreach ( $deactivated_plugins as $plugin ) {
		if ( ! empty( $plugin['version_compatible'] ) && ! empty( $plugin['version_deactivated'] ) ) {
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with WLP %3$s, please upgrade to %1$s %4$s or later.' ),
				$plugin['plugin_name'],
			...
			$explanation = sprintf(
				...
				__( '%1$s %2$s was deactivated due to incompatibility with WLP %3$s.' ),
				$plugin['plugin_name'],
		...
		printf(
			...
			sprintf(
				...
				__( '%s plugin deactivated during WLP upgrade.' ),
				$plugin['plugin_name']
			),
			$explanation,
