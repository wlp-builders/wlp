...
function delete_theme( $stylesheet, $redirect = '' ) {
	...
	if ( empty( $themes_dir ) ) {
		return new WP_Error( 'fs_no_themes_dir', __( 'Unable to locate WLP theme directory.' ) );
	}
				...
				<div class="theme-info">
							...
							<div class="notice notice-error notice-alt notice-large" data-slug="{{ data.id }}">
								...
								<p>
									<# if ( ! data.updateResponse.compatibleWP && ! data.updateResponse.compatiblePHP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your versions of WLP and PHP.' ),
											'{{{ data.name }}}'
										);
										if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
											if ( $cp_needs_update ) {
												printf(
													...
													' ' . __( '<a href="%1$s">Please update WLP</a>, and then <a href="%2$s">learn more about updating PHP</a>.' ),
											...
											printf(
												...
												' ' . __( '<a href="%s">Please update WLP</a>.' ),
												self_admin_url( 'update-core.php' )
											);
									...
									<# } else if ( ! data.updateResponse.compatibleWP ) { #>
										...
										printf(
											/* translators: %s: Theme name. */
											__( 'There is a new version of %s available, but it does not work with your version of WLP.' ),
											'{{{ data.name }}}'
										);
					...
					<# if ( ! data.compatibleWP || ! data.compatiblePHP ) { #>
						<div class="notice notice-error notice-alt notice-large"><p>
							<# if ( ! data.compatibleWP && ! data.compatiblePHP ) { #>
								<?php
								_e( 'This theme does not work with your versions of WLP and PHP.' );
								if ( current_user_can( 'update_core' ) && current_user_can( 'update_php' ) ) {
									if ( $cp_needs_update ) {
										printf(
											/* translators: 1: URL to WordPress Updates screen, 2: URL to Update PHP page. */
							...
							<# } else if ( ! data.compatibleWP ) { #>
								<?php
								_e( 'This theme does not work with your version of WLP.' );
								if ( current_user_can( 'update_core' ) && $cp_needs_update ) {
									printf(
										/* translators: %s: URL to WordPress Updates screen. */
										' ' . __( '<a href="%s">Please update WLP</a>.' ),
