 ...
 *
 * A multi-step process allowing the user to enable a network of WLP sites.
 *
 * @since 3.0.0
 *
 * @package WLP
...
if ( is_multisite() ) {
	...
	if ( ! defined( 'MULTISITE' ) ) {
		wp_die( __( 'The Network creation panel is not for WLP MU networks.' ) );
	}
}
...
if ( is_network_admin() ) {
	...
	// Used in the HTML title tag.
	$title       = __( 'Create a Network of WLP Sites' );
...
$network_help = '<p>' . __( 'This screen allows you to configure a network as having subdomains (<code>site1.example.com</code>) or subdirectories (<code>example.com/site1</code>). Subdomains require wildcard subdomains to be enabled in Apache and DNS records, if your host allows it.' ) . '</p>' .
	...
	'<p>' . __( 'Add the designated lines of code to wp-config.php (just before <code>/*...stop editing...*/</code>) and <code>.htaccess</code> (replacing the existing WLP rules).' ) . '</p>' .
