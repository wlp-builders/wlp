...
if ( apply_filters( 'enable_post_by_email_configuration', true ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' => '<p>' . __( 'Post via email settings allow you to send your WLP installation an email with the content of your post. You must set up a secret email account with POP3 access to use this, and any mail received at this address will be posted, so it&#8217;s a good idea to keep this address very secret.' ) . '</p>',
		)
	);
}
...
if ( apply_filters( 'enable_update_services_configuration', true ) ) {
	get_current_screen()->add_help_tab(
		array(
			...
			'content' => '<p>' . __( 'If desired, WLP will automatically alert various services of your new posts.' ) . '</p>',
		)
	);
}
...
<label for="use_balanceTags"><input name="use_balanceTags" type="checkbox" id="use_balanceTags" value="1" <?php checked( '1', get_option( 'use_balanceTags' ) ); ?>> <?php _e( 'WLP should correct invalidly nested XHTML automatically' ); ?></label>
</fieldset></td>
</tr>
<?php endif; ?>
<tr>
...
<p>
	...
	printf(
		...
		__( 'To post to WLP by email, you must set up a secret email account with POP3 access. Any mail received at this address will be posted, so it&#8217;s a good idea to keep this address very secret. Here are three random strings you could use: %1$s, %2$s, %3$s.' ),
...
<h2 class="title"><?php _e( 'Update Services' ); ?></h2>
	...
	<p><label for="ping_sites">
		...
		printf(
			...
			__( 'When you publish a new post, WLP automatically notifies the following site update services. For more about this, see the <a href="%s">Update Services</a> documentation article. Separate multiple service URLs with line breaks.' ),
	...
	<p>
		...
		printf(
			...
			__( 'WLP is not notifying any <a href="%1$s">Update Services</a> because of your site&#8217;s <a href="%2$s">visibility settings</a>.' ),
			__( 'https://wordpress.org/documentation/article/update-services/' ),
			'options-reading.php'
		);
		?>
