...
/** Load WLP Administration Upgrade API */
require_once ABSPATH . 'wp-admin/includes/upgrade.php';

/** Load WLP Translation Installation API */
...
if ( file_exists( ABSPATH . 'wp-config-sample.php' ) ) {
	...
	wp_die(
		sprintf(
			...
			__( 'Sorry, I need a %s file to work from. Please re-upload this file to your WLP installation.' ),
...
if ( @file_exists( ABSPATH . '../wp-config.php' ) && ! @file_exists( ABSPATH . '../wp-settings.php' ) ) {
	wp_die(
		'<p>' . sprintf(
			/* translators: 1: wp-config.php, 2: install.php */
			__( 'The file %1$s already exists one level above your WLP installation. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href="%2$s">installing now</a>.' ),
			'<code>wp-config.php</code>',
			'install.php'
		) . '</p>',
		409
...
<head>
	...
	<title><?php _e( 'WLP &rsaquo; Setup Configuration File' ); ?></title>
	<?php wp_admin_css( 'install', true ); ?>
</head>
<body class="<?php echo implode( ' ', $body_classes ); ?>">
<p id="logo"><?php _e( 'WLP' ); ?></p>
	<?php
...
<p><?php _e( 'Welcome to WLP. Before getting started, you will need to know the following items.' ); ?></p>
<ol>
	<li><?php _e( 'Database name' ); ?></li>
	<li><?php _e( 'Database username' ); ?></li>
	<li><?php _e( 'Database password' ); ?></li>
	<li><?php _e( 'Database host' ); ?></li>
	<li><?php _e( 'Table prefix (if you want to run more than one WLP in a single database)' ); ?></li>
</ol>
<p>
		<?php
		printf(
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table" role="presentation">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" aria-describedby="dbname-desc" size="25" placeholder="wlp"<?php echo $autofocus; ?>></td>
			<td id="dbname-desc"><?php _e( 'The name of the database you want to use with WLP.' ); ?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td><input name="prefix" id="prefix" type="text" aria-describedby="prefix-desc" value="wlp_" size="25"></td>
			<td id="prefix-desc"><?php _e( 'If you want to run multiple WLP installations in a single database, change this.' ); ?></td>
		</tr>
	</table>
...
<div class="container">
    <div class="column">
        <h3>Shared Hosting Panel Steps</h3>
        <pre id="output-shared"></pre>
    </div>
    <hr>
    <div class="column">
        <h3>Latest Linux Setup</h3>
        <pre id="output-linux"></pre>
    </div>
</div>

<!-- WLP automatic steps generation -->
<script>
...
4. Create a new database named \`${dbname}\`.
5. Create a new user named '${uname}' with password '${pwd}'.
6. Add the user to the database and grant all privileges.
7. Note down the database host, usually 'localhost'.
8. Update your application configuration with the following:
   ...
   Table Prefix: ${prefix}
        ...
        const linuxSteps = `
# Latest Linux Setup
1. Open the terminal and log in to MySQL:
   sudo mysql
...
3. Exit MySQL:
   EXIT;
        ...
        // Output commands
        document.getElementById('output-shared').textContent = sharedSteps;
        document.getElementById('output-linux').textContent = linuxSteps;
    };
	...
	case 2:
		...
		*/
		...
		try {
			...
			if ( ! $no_api ) {
				$secret_keys = wp_remote_get( 'https://api.wlp.net/secret-key/1.0/salt/' );
			}
...
<p><?php _e( 'All right, sparky! You&#8217;ve made it through this part of the installation. WLP can now communicate with your database. If you are ready, time now to&hellip;' ); ?></p>
