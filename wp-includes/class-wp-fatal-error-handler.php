...
class WP_Fatal_Error_Handler {
	...
	public function handle() {
		...
		try {
            ...
            // WLP supports removal of recovery mode
			if (class_exists('WP_Recovery_Mode_Link_Service') && wp_recovery_mode()->is_initialized() ) {
	...
	protected function display_default_error_template( $error, $handled ) {
		...
		} elseif ( class_exists('WP_Recovery_Mode_Link_Service') && is_protected_endpoint() && wp_recovery_mode()->is_initialized() ) {
            // WLP supports removal of WP Recovery mode
		...
		$message = sprintf(
			...
			__( 'Learn more about troubleshooting WLP.' )
		);
