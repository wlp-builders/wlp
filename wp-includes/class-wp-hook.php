	...
	public function apply_filters( $value, $args ) {
		...
		do {
			...
			foreach ( $this->callbacks[ $priority ] as $the_ ) {
				// <!-- in WLP we can disable things like the api
				if(!function_exists('rest_api_init') && $the_['function'] === 'rest_api_init') {
					continue;
				}
				if(!function_exists('rest_cookie_collect_status') && $the_['function'] === 'rest_cookie_collect_status') {
					continue;
				}
				if(!function_exists('rest_api_loaded') && $the_['function'] === 'rest_api_loaded') {
					continue;
				}
				if(!function_exists('rest_output_link_header') && $the_['function'] === 'rest_output_link_header') {
					continue;
				}
				if(!function_exists('rest_output_link_wp_head') && $the_['function'] === 'rest_output_link_wp_head') {
					continue;
				}

				if($the_['function'] === 'wp_widgets_init' && !class_exists('WP_Widget_Factory')) {
                    // in WLP you can disable widgets
                    continue;
                }
				if(!function_exists('wp_sitemaps_get_server') && $the_['function'] === 'wp_sitemaps_get_server') {
                    // in WLP you can disable sitemaps
					continue;
				}
				if(!function_exists('wp_recovery_mode_nag') && $the_['function'] === 'wp_recovery_mode_nag') {
                    // in WLP you can disable sitemaps
					continue;
				}
				if(!function_exists('update_nag') && $the_['function'] === 'update_nag') {
                    // in WLP you can disable sitemaps
					continue;
				}
				if(!function_exists('maintenance_nag') && $the_['function'] === 'maintenance_nag') {
                    // in WLP you can disable sitemaps
					continue;
				}

				if(!function_exists('wp_theme_update_rows') && $the_['function'] === 'wp_theme_update_rows') {
					continue;
				}
