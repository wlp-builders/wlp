...
WLP has a built-in feature that detects when a plugin or theme causes a fatal error on your site, and notifies you with this automated email.
###CAUSE###
...
###DETAILS###'
	...
	private function get_cause( $extension ) {
		...
		if ( 'plugin' === $extension['type'] ) {
			...
			$cause = sprintf( __( 'In this case, WLP caught an error with one of your plugins, %s.' ), $name );
		} else {
			$theme = wp_get_theme( $extension['slug'] );
			$name  = $theme->exists() ? $theme->display( 'Name' ) : $extension['slug'];

			/* translators: %s: Theme name. */
			$cause = sprintf( __( 'In this case, WLP caught an error with your theme, %s.' ), $name );
		}
