...
class wpdb {
	...
	* @since CP-2.2.0 - Added `object_relationships` and `object_relationshipmeta`
	...
	/**
	 * WLP Object Relationships table.
	 *
	 * @since CP-2.2.0
	 *
	 * @var string
	...
	/**
	 * WLP Object Relationship Meta table.
	 *
	 * @since CP-2.2.0
	 *
	 * @var string
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: Support forums URL. */
				__( 'If you are unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">WLP support forums</a>.' ),
				__( 'https://wordpress.org/support/forums/' )
			) . "</p>\n";
	...
	public function query( $query ) {
		...
		if ( $this->check_current_query && ! $this->check_ascii( $query ) ) {
			...
			if ( $stripped_query !== $query ) {
				...
				$this->last_error = __( 'WLP database error: Could not perform query because it contains invalid data.' );
		...
		if ( $data !== $converted_data ) {
			...
			if ( 1 === count( $problem_fields ) ) {
				$this->last_error = sprintf(
					...
					__( 'WLP database error: Processing the value for the following field failed: %s. The supplied value may be too long or contains invalid data.' ),
					reset( $problem_fields )
				);
			} else {
				$this->last_error = sprintf(
					/* translators: %s: Database fields where the error occurred. */
					__( 'WLP database error: Processing the values for the following fields failed: %s. The supplied values may be too long or contain invalid data.' ),
	...
	public function check_database_version() {
		...
		if ( version_compare( $this->db_version(), $required_mysql_version, '<' ) ) {
			...
			return new WP_Error( 'database_version', sprintf( __( '<strong>Error:</strong> WLP %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ) );
		}
	}
