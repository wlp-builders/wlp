...
function _wp_die_process_input( $message, $title = '', $args = array() ) {
	...
	if ( empty( $title ) ) {
		$title = $have_gettext ? __( 'WLP &rsaquo; Error' ) : 'WordPress &rsaquo; Error';
	}
	...
	if ( WP_DEBUG && apply_filters( 'doing_it_wrong_trigger_error', true, $function_name, $message, $version ) ) {
		if ( function_exists( '__' ) ) {
			...
			$message .= ' ' . sprintf(
				...
				__( 'Please see <a href="%s">Debugging in WLP</a> for more information.' ),
 ...
 *  but not if WLP_ENABLE_IFRAME for developers is enabled
 * @since 2.0.0 (WLP)
 *
 * @see https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/X-Frame-Options
 */
function send_frame_options_header() {
    if(!(defined('WLP_ENABLE_IFRAME') && WLP_ENABLE_IFRAME)) {
...
function wp_auth_check_html() {
    return false; // JWT tokens should automatically refresh using AJAX when inside the wp-admin
    // so theres no need to reauth.. 
...
function wp_get_update_php_annotation() {
	...
	$annotation = sprintf(
		...
		__( 'This resource is provided by your web host, and is specific to your site. For more information, <a href="%s" target="_blank">see the official WLP documentation</a>.' ),
		esc_url( $default_url )
	);
