...
function register_admin_color_schemes() {
	...
	wp_admin_css_color(
		...
		false,
		array( '#1d2327', '#2c3338', '#a91bbe', '#f6aaff' ),
		array(
			'base'    => '#a7aaad',
			'focus'   => '#f6aaff',
			'current' => '#fff',
...
function get_the_generator( $type = '' ) {
	...
	switch ( $type ) {
		case 'html':
			$gen = '<meta name="generator" content="WordPress ' . esc_attr( get_bloginfo( 'version' ) ) . ' (compatible; WLP ' . esc_attr( classicpress_version_short() ) . ')">';
			break;
		case 'atom':
			$gen = '<generator uri="https://www.classicpress.net/" version="' . esc_attr( get_bloginfo( 'version' ) ) . '-cp-' . esc_attr( classicpress_version_short() ) . '">WLP</generator>';
			break;
		case 'rss2':
		...
		case 'rdf':
			...
			break;
		case 'comment':
			$gen = '<!-- generator="WordPress/' . esc_attr( get_bloginfo( 'version' ) ) . '(compatible; WLP/' . esc_attr( classicpress_version_short() ) . ')" -->';
			break;
		case 'export':
			$gen = '<!-- generator="WLP/' . esc_attr( classicpress_version_short() ) . '" created="' . esc_attr( date( 'Y-m-d H:i' ) ) . '" -->';
			break;
	}
 ...
 * showing the WLP logo (the default) or the site's custom login image
 * (if enabled).
 *
 * @since 1.2.0
 */
function get_login_image_html() {
	...
	if ( ! empty( $login_custom_image_html ) ) {
		...
		$login_header_title = __( 'Powered by WLP' );
	}
