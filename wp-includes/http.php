 ...
 *
 * Standardizes the HTTP requests for WLP. Handles cookies, gzip encoding and decoding, chunk
...
/**
 * Returns the WLP User-Agent header for outgoing requests.
 *
 ...
 * come from WordPress, WLP uses the same pattern, but uses the URL to
 * indicate that requests come from a WLP site.
 *
 * @since CP-1.0.0
 *
 * @param bool $include_site_id Whether to include a site identifier (used when
 *                              communicating with the WLP API).  This
...
function classicpress_user_agent( $include_site_id = false ) {
	$url = 'https://whitelabelpress.org/?wp_compatible=true'; // WLP removes version nr for privacy
	...
	$user_agent = 'WLP/' . get_bloginfo( 'version' ) . '; ' . $url;
	 ...
	 *
	 * @param string $user_agent WLP user agent string.
	 */
