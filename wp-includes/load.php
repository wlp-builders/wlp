<?php
/**
 * These functions are needed to load WLP.
 *
 * @package WLP
 */
...
function wp_check_php_mysql_versions() {
	...
	if ( version_compare( $required_php_version, $php_version, '>' ) ) {
		...
		printf(
			'Your server is running PHP version %1$s but WLP %2$s requires at least %3$s.',
			$php_version,
			$wp_version,
	...
	if ( ! function_exists( 'mysqli_connect' )
		...
		$message = '<p>' . __( 'Your PHP installation appears to be missing the MySQL extension which is required by WLP.' ) . "</p>\n";
...
function wp_is_recovery_mode() {
    // WLP lets you remove recovery mode
