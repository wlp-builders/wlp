...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that your host&#8217;s database server is not running, WLP was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	$msg .= sprintf(
		...
		__( 'Read the <a href="%s" target="_blank">Debugging a WLP Network</a> article. Some of the suggestions there may help you figure out what went wrong.' ),
