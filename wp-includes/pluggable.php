...
if ( ! function_exists( 'wp_set_current_user' ) ) :
	 ...
	 *
	 * Some WLP functionality is based on the current user and not based on
...
if ( ! function_exists( 'wp_validate_auth_cookie' ) ) :
	 ...
	 *
	 * The checks include making sure that the authentication JWT is correct.
	 *
	 * Makes sure the JWT is not expired. 
	 *
	 * @since 1.1.0 (WLP)
	 *
	 * @return int|false User ID if valid JWT, false if invalid.
	 */
	function wp_validate_auth_cookie() {
		// Verify JWT here (user_id), with JWT no need for a manager/state save
		$user_id = null;
		try { 
			$token = wlp_get_jwt();
...
if ( ! function_exists( 'wp_generate_auth_cookie' ) ) :
	 ...
	 */
	function wp_generate_auth_cookie( $user_id, $expiration, $scheme = 'auth', $tokenold = '' ) {
		$user = get_userdata( $user_id );
		if ( ! $user ) {
			return '';
		}

		$cookie = wlp_jwt_sign(['user_id' => $user_id], WLP_JWT_SECRET,$expiration);
...
if ( ! function_exists( 'wp_set_auth_cookie' ) ) :
	 ...
	 * @param string      $tokenold    Optional. User's session token to use for this cookie.
	 */
	function wp_set_auth_cookie( $user_id, $remember = false, $secure = '', $tokenold = '' ) {
...
if ( ! function_exists( 'wp_clear_auth_cookie' ) ) :
	...
	function wp_clear_auth_cookie() {
		...
		// Auth cookies.
		setcookie( 'jwt_token', ' ', time() - YEAR_IN_SECONDS, '/', COOKIE_DOMAIN );
...
if ( ! function_exists( 'auth_redirect' ) ) :
	...
	function auth_redirect() {
		...
		if ( $user_id ) {
			//die('died with valid $user_id (good)');
			/**
