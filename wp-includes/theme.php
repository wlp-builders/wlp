	...
	if ( ! $compatible_wp && ! $compatible_php ) {
		return new WP_Error(
			...
			sprintf(
				...
				_x( '<strong>Error:</strong> Current WLP and PHP versions do not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
		...
		return new WP_Error(
			...
			sprintf(
				/* translators: %s: Theme name. */
				_x( '<strong>Error:</strong> Current WLP version does not meet minimum requirements for %s.', 'theme' ),
				$theme->display( 'Name' )
			)
		);
	}
...
function register_theme_feature( $feature, $args = array() ) {
	...
	if ( is_array( $args['show_in_rest'] ) ) {
		...
		// <!-- WLP allows to disable REST API completely without breaking this file.
		if ( function_exists( 'rest_default_additional_properties_to_false' ) ) {
			...
			$args['show_in_rest']['schema'] = false; // Default to false if not defined.
		}
