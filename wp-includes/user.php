...
function wp_update_user( $userdata ) {
	...
	if ( ! empty( $userdata['user_pass'] ) && $userdata['user_pass'] !== $user_obj->user_pass ) {
		...
		$userdata['user_pass'] = password_hash($userdata['user_pass'],PASSWORD_BCRYPT);
...
function _wp_get_current_user() {
	...
	/**
	 * Simply set current user if JWT matches
	 *
	 * @since 1.1.0 (WLP)
	 */
	// Verify JWT here (user_id), no need for a manager/state save
	$user_id = null;
	try { 
		...
		//die('died from user.php jwt bug (bad)');
		//var_dump('died from user.php jwt bug (bad)');

		return false;
	}

	if ( ! $user_id ) {
		//die('died from user.php jwt bug user_id not set (bad)');
		//var_dump('died from user.php jwt bug user_id not set (bad)');
...
function wp_register_user_personal_data_exporter( $exporters ) {
	$exporters['wordpress-user'] = array(
		'exporter_friendly_name' => __( 'WLP User' ),
...
function wp_user_personal_data_exporter( $email_address ) {
	...
	if ( isset( $user_meta['community-events-location'] ) ) {
		...
		$data_to_export[] = array(
			...
			'group_description' => __( 'User&#8217;s location data used for the Community Events in the WLP Events and News dashboard widget.' ),
