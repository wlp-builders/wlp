...
/** Make sure that the WLP bootstrap has run before continuing. */
...
function login_header( $title = 'Log In', $message = '', $wp_error = null ) {
	...
	$login_title = sprintf( __( '%1$s &lsaquo; %2$s &#8212; WLP' ), $title, $login_title );
...
/* custom dark css default whitelabelpress */
body,input,textarea,.login form,.login form .input {
background:black;
color:white;
}
...
$default_actions = array(
	...
	'confirmaction',
	'login',
    class_exists('WP_Recovery_Mode_Link_Service') ? WP_Recovery_Mode_Link_Service::LOGIN_ACTION_ENTERED : '',
);
	...
	default:
		...
		if ( $interim_login ) {
			...
			if ( isset( $_GET['loggedout'] ) && $_GET['loggedout'] ) {
				...
				$errors->add( 'updated', __( '<strong>You have successfully updated WLP!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
			} elseif ( class_exists('WP_Recovery_Mode_Link_Service') && WP_Recovery_Mode_Link_Service::LOGIN_ACTION_ENTERED === $action ) {
