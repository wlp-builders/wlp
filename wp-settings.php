<?php
define('WLP',1); // major version
/**
 * Used to set up and fix common variables and include
 * the WLP procedural and class library.
 *
...
/**
 * Stores the location of the WLP directory of functions, classes, and core content.
 *
 * @since 1.0.0
 */
if ( ! defined( 'WPINC' ) ) {
...
// <-- WLP first loads 'core plugins' (begin)
// Define the absolute path to the core plugins directory outside wp-content
$plugin_dir = ABSPATH . 'wlp-core-plugins/enabled/';
// Check if the directory exists
if ( is_dir( $plugin_dir ) ) {

	// Get all PHP files in the core plugins folder, including subdirectories if needed
    $plugin_files = glob( $plugin_dir . '**/*.php', GLOB_BRACE ); // This will get all PHP files, including in subfolders
    
	// Custom sorting for alphanumeric order (numbers first, then letters)
	

    // Loop through each plugin file and include it
    foreach ( $plugin_files as $plugin_file ) {
		// Exclude if disabled
		// skip this menu if set to disabled
		if(defined('WLP_CORE_PLUGINS_DISABLE')) {
			$bname = basename($plugin_file);
			if(in_array($bname, WLP_CORE_PLUGINS_DISABLE)) {
				return false;
			}
		}

		//echo "requiring ".$plugin_file.PHP_EOL;

        // Include the plugin file
        require_once( $plugin_file );
    }
}
// <-- WLP first loads 'core plugins' (end) 
...
// WLP calculates offsets from UTC.
...
// Start the WLP object cache, or an external object cache if the drop-in is present.
wp_start_object_cache();

// Attach the default filters.
require_once ABSPATH . WPINC . '/default-filters.php';

// <-- in WLP multisite is disabled but the variable is kept for backwards compatibility
define( 'MULTISITE', false );

register_shutdown_function( 'shutdown_action_hook' );

// Stop most of WLP from being loaded if SHORTINIT is enabled.
if ( SHORTINIT ) {
	return false;
}
...
// Run the installer if WLP is not installed.
wp_not_installed();
...
require_once ABSPATH . WPINC . '/object-relationships.php';
...
if ( 0 !== (int) get_option( 'blocks_compatibility_level', 1 ) ) {
	require_once ABSPATH . WPINC . '/classicpress/class-wp-compat.php';
}
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	require_once ABSPATH . WPINC . '/class-fix-wpcli.php';
}
...
// WLP allows you to remove recovery mode
if ( class_exists('WP_Recovery_Mode') && wp_is_fatal_error_handler_enabled() ) {
 ...
 * Use this global for WLP queries
 *
...
/**
 * Holds the WLP Rewrite object for creating pretty URLs
 *
...
/**
 * WLP Widget Factory Object
 *
 * @global WP_Widget_Factory $wp_widget_factory
 * @since 2.8.0
 */
if(class_exists('WP_Widget_Factory')) {
    // <!-- in WLP you can disable widgets
...
/**
 * WLP Locale object for loading locale domain date and various strings.
 *
...
/**
 *  WLP Locale Switcher object for switching locales.
 *
 * @since 4.7.0
 *
...
/**
 * Fires after WLP has finished loading but before any headers are sent.
 *
